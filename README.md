...
**反驳者拿最强的** —— 软反驳者等于没有反驳者;**实现者紧跟** —— 它才是真正要写代码的那个;
复核者最后。

聊天里想换模型就点选:开跑前的确认里选「改模型」,一题一个角色摆候选,
**推荐的排第 1 位**并带一句按角色给的理由(反驳者最强、实现者紧跟、复核者最省)。
...
| 角色 | 子 agent | 模型 | 工具 |
|---|---|---|---|
| 实现者 | `forge-proposer` | `sonnet` | 读写 + Bash |
...
|---|---|
| 开局：给目标、判据、预算、角色 | ❌ **判达标** |
| 推进轮次、把上一轮的失败信息带下去 | ❌ 自己写代码（那是实现者的活） |
...
所以 codex 那边看不到「实现者花了多少、反驳者花了多少」—— 分不开就不分，
...
─ 逐 agent ───────────────────────────────────────────────────────────────────
  ...
  实现者       sonnet-4-5        8.0k    1.2k     45.0k   R1,R2   Read×9 Edit×4
...
```
node test-host.js    # 125 项：宿主驱动 + 跨宿主适配器 + 逐 agent 用量 + MCP 全链路 + 插件包装
